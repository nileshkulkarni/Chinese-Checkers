     ...
     ; ((draw-string  view-port) (make-posn 20 50) "Current  Player")
      ...
      (let ([y (evaluate-board player)])
        ...
        ;  (display "wtf")
          ...
          (make-move a)
          (let([t (get-n-ind i)])
            (if (game-win (color->index (send player get-color))) 
                (begin
                               ((draw-solid-rectangle view-port) (make-posn 0 0) 1000 800 "cyan")
                               (sleep 2)
                               ((draw-pixmap view-port) (string-append (symbol->string (send player get-color)) "1.jpg") (make-posn 200 200))                               
