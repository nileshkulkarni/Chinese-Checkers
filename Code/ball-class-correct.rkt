...
;     
;        (set! a (filter (lambda(t) 
;                         (equal? (send (send cur-board get-elm (- (* 2 (car t)) (car ind)) 
;                                                (- (* 2 (cdr t)) (cdr ind))) get-state) 'null)) a))
;      (set! mlist (append (map (lambda(t) (cons t 2)) a) mlist))
;      (map (lambda(z) (helper (cons
;                                  (- (* 2 (car z)) (car ind)) 
;                                  (- (* 2 (cdr z)) (cdr ind))))) a))))
;        
;     
