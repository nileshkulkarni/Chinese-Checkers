...
(define (search-ball ind player);;;returns the ball along with its current index (cons index ball)
  (let* ([ball-vec (send  player get-all-balls)] ;remove this
         [all-indexes (vector-map (lambda (x) (cons (send x get-index) x)) ball-vec)])
    
    (cond [(equal? #f (vector-assq all-indexes ind)) (error "The ball does not exist at that location")]  ;remove the display statement
  ...
  (define (helper mlist)
    (if (null? mlist) (void)
        (let*([move (car mlist)]
              ...
              [ball (cdr (search-ball from (move_-player (car mlist))))]);(car move..replaces move
          (begin
            (send ball index-set! to)
            (helper (cdr mlist))))))
...
(include "game-win.scm")
(include "assign-players.scm")


(define view-port (open-viewport "Chinese Checkers" (make-posn (* 5 l) 750)))

(define option (open-viewport "Options Window" (make-posn 768 480)))

((draw-pixmap view-port) "chi.jpg" (make-posn 0 0))
((draw-solid-rectangle view-port) (make-posn 200 0) 600 750 "LightCyan")
((draw-pixmap view-port) "checkers.jpg" (make-posn 800 0) )

(include "AI_bit_modified.scm")
